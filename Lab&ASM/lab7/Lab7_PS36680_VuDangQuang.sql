...
-- Thêm 1 khách hàng mới (thông tin tự cho).
insert into khachhang
values ('KH025', 'Vu Dang', 'Quang', 'Tân Bình', '', '01666331161');

-- Thêm 3 sản phẩm mới (search tìm thông tin sản phẩm)
insert into sanpham
values ('SP007', 'Iphone XS Max', 6000000.0, 10, 'Hàng xách tay'),
('SP008', 'Nokia 1280', 249000.0, 1, 'Hàng chính hãng'),
('SP009', 'Huawei P40 Pro+', 24990000.0, 100, 'Hàng chính hãng');

-- Thêm 1 hóa đơn mới cho khách hàng mới thêm
insert into hoadon
values ('HD030', '2022-09-07', 30990000, 'Chưa thanh toán', 'NV001', 'KH025');

-- Thêm hóa đơn chi tiết của hóa đơn vừa nhập mua 3 sản phẩm bất kỳ.
insert into hoadonchitiet
values ('HD006', 'SP007', 1),
('HD006', 'SP009', 2),
('HD006', 'SP001', 7);

-- Tạo 1 bảng có tên KhachHang_Q1 chứa thông tin tin đầy đủ về các khách hàng ở Quận 1
...
-- Tăng số lượng mặt hàng có mã ‘SP003’ lên thêm ‘200’ đơn vị
update sanpham
...
-- Tăng số lượng của mặt hàng bán chạy nhất trong tháng 2/2019 lên 100 đơn vị
update sanpham
set soluong = soluong + 100
where masp in
	(select masp
    from 
		(select masp, sum(soluongmua) as soluonghangban
		from hoadon hd inner join hoadonchitiet hdct on hd.mahd = hdct.mahd
		where month(ngaylaphd) = 2 and year(ngaylaphd) = 2019
		group by masp
		having soluonghangban =
			(select max(soluonghangban)
            from
				(select sum(soluongmua) as soluonghangban
                from hoadon hd inner join hoadonchitiet hdct on hd.mahd = hdct.mahd
                where month(ngaylaphd) = 2 and year(ngaylaphd) = 2019
                group by masp) as soluongmua
			)
		) as maspbanchaynhat
	);
    
-- Giảm giá 10% cho 2 sản phẩm bán ít nhất trong năm 2019
update sanpham
set giasp = giasp * 0.9
where masp in
	(select masp
    from
		(select masp, sum(soluongmua)
        from hoadonchitiet hdct inner join hoadon hd on hdct.mahd = hd.mahd
        where year(ngaylaphd) = 2019
        group by masp
        order by sum(soluongmua) asc
        limit 2) as maspbanit2019
	);
    
-- Cập nhật lại trạng thái “chưa thanh toán” cho hoá đơn có mã ‘HD001’
update hoadon
set trangthai = 'chưa thanh toán'
where mahd = 'HD001';

-- Xoá mặt hàng có mã sản phẩm là ‘SP001’ ra khỏi hoá đơn ‘HD001’ .
delete from hoadonchitiet
where masp = 'sp001' and mahd = 'hd001';

-- Xoá khách hàng chưa từng mua hàng kể từ ngày “1-1-2019”
alter table hoadon
drop foreign key fk_hd2;
